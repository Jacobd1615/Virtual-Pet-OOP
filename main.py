...
class Pet:
    ...
    def feed(self):
        ...
        if self.hunger >= 100:
            return (
                False,
                f"{Fore.RED}💥 {self.name} was overfed and EXPLODED! Guts everywhere...{Style.RESET_ALL}\n{Fore.RED}☠️ Game over. You monster.{Style.RESET_ALL}",
            )
        self.hunger = max(0, min(self.hunger + 10, 100))  # Increase hunger by 10
        message = (
            f"{Fore.RED}😈 {self.name} 'I'm fine do not over feed me!'{Style.RESET_ALL}"
            if self.feed_count > 3
            else f"{Fore.GREEN}🍽️ {self.name} has been fed!{Style.RESET_ALL} Hunger: {self.hunger}/100, Happiness: {self.happiness}/100 😊"
        )
    ...
    def play(self):
        if self.play_locked:
            return (
                True,
                f"{Fore.RED}😡 {self.name} snaps, 'I said STOP! You’re done playing with me.'{Style.RESET_ALL}",
            )
    ...
    def bath(self):
        ...
        if outcome == outcomes[0]:  # Nice bath
            self.happiness = max(
                0, min(self.happiness + 10, 100)
            )  # Increase happiness by 10
            continue_game = True
            status_message = self._check_status()[1]
            return (
                continue_game,
                f"{outcome} Happiness: {self.happiness}/100 😊\n{status_message}",
            )
    ...
    def _check_status(self):
        ...
        if self.hunger == 100:
            ...
            return (
                False,
                f"{Fore.RED}😈 {self.name}’s hunger consumes you! 'Your pet is literally eating your bones....'{Style.RESET_ALL}\n{Fore.RED}☠️ Game over. You’ve been eaten.{Style.RESET_ALL}",
            )
        elif self.hunger <= 20:
        ...
        if self.happiness == 0:
            return (
                False,
                f"{Fore.RED}☠️ {self.name} glares with hatred and vanishes into the night! 'Leaving an axe in your face....'{Style.RESET_ALL}\n{Fore.RED}☠️ Game over. You’re alone now.{Style.RESET_ALL}",
            )
    ...
    def status(self):
        ...
        hunger_color = (
            Fore.GREEN
            if self.hunger <= 50
            else Fore.YELLOW if self.hunger <= 80 else Fore.RED
        )
        happiness_color = (
            Fore.GREEN
            if self.happiness >= 50
            else Fore.YELLOW if self.happiness >= 20 else Fore.RED
        )
...
class Dog(Pet):
    ...
    def learn_trick(self):
        ...
        if self.happiness >= 50 and self.hunger <= 50:
            ...
            message = (
                f"{Fore.YELLOW}🐶 {random.choice(trick_messages)}{Style.RESET_ALL}"
            )
            continue_game, status_message = self._check_status()
            return continue_game, f"{message}\n{status_message}"
        else:
            return (
                True,
                f"{Fore.RED}😣 {self.name} snarls, 'I’m not your puppet!'{Style.RESET_ALL}",
            )
...
class Cat(Pet):
    ...
    def scratch(self):
        ...
        self.action_count += 1  # Increment action count
        self.happiness = max(0, min(self.happiness - 5, 100))  # Decrease happiness by 5
        if random.random() < 0.3:  # 30% chance to scratch player
            ...
            if choice == "yell":
                ...
                return (
                    True,
                    f"{Fore.RED}😡 You yell at {self.name}! It hisses, eyes full of hate. Happiness: 1/100 😿{Style.RESET_ALL}\n{self._check_status()[1]}",
                )
            elif choice == "comfort":
                self.happiness = 90
                return (
                    True,
                    f"{Fore.GREEN}🤗 You comfort {self.name}. It purrs softly, forgiving you. Happiness: 90/100 😺{Style.RESET_ALL}\n{self._check_status()[1]}",
                )
            else:
                return (
                    True,
                    f"{Fore.RED}❌ Invalid choice. {self.name}’s claws gleam... Happiness: {self.happiness}/100 😿{Style.RESET_ALL}\n{self._check_status()[1]}",
                )
        ...
        message = (
            f"{Fore.RED}😾 {self.name} slashes wildly! 'Next is your throat.' Scratch count: {self.scratch_count} 🩸{Style.RESET_ALL}"
            if self.scratch_count > 3
            else f"{Fore.YELLOW}😺 {self.name} scratched something! Scratch count: {self.scratch_count} 🐾{Style.RESET_ALL}"
        )
        continue_game, status_message = self._check_status()
        return continue_game, f"{message}\n{status_message}"
...
def display_rules():
    ...
    print(f"{Fore.RED}  - Use bath (It might go well){Style.RESET_ALL}")
...
def display_menu(pet, activity_log=""):
    ...
    if (
        pet.action_count >= 3 and random.random() < 0.1
    ):  # 10% chance for demonic art after 3 actions
        pet_art = pet_art_list[1]  # Demonic
    elif (
        pet.happiness <= 20 and random.random() < 0.3
    ):  # 30% chance for normal art when happiness <= 20
    ...
    print(f"{Fore.CYAN}╠════════════════════════════════════════════╣{Style.RESET_ALL}")
    print(f"{Fore.CYAN}║  🍽️ [1]   - Feed your pet                ║{Style.RESET_ALL}")
    print(f"{Fore.CYAN}║  🎾 [2]   - Play with your pet            ║{Style.RESET_ALL}")
    print(f"{Fore.CYAN}║  📊 [3]   - Show pet status               ║{Style.RESET_ALL}")
    print(f"{Fore.CYAN}║   [4]   - Give your pet a bath          ║{Style.RESET_ALL}")
    if isinstance(pet, Dog):
        print(
            f"{Fore.CYAN}║  🏆 [5]   - Teach your dog a trick        ║{Style.RESET_ALL}"
        )
    elif isinstance(pet, Cat):
        print(
            f"{Fore.CYAN}║  🐾 [5]   - Let your cat scratch          ║{Style.RESET_ALL}"
        )
    print(f"{Fore.CYAN}║  🚪 [6]   - Quit the game                 ║{Style.RESET_ALL}")
    print(f"{Fore.CYAN}╚════════════════════════════════════════════╝{Style.RESET_ALL}")
    print(f"\n{pet_art}\n")
    if activity_log:
...
def main():
    ...
    while True:
        option = (
            input(f"{Fore.MAGENTA}Choose your pet: 🐶 dog or 😺 cat?{Style.RESET_ALL} ")
            .strip()
            .lower()
        )
    ...
    print(
        f"{Fore.GREEN}☠️ You’ve unleashed a {option} named {pet.name}! It knows your fears... 🐾{Style.RESET_ALL}"
    )
    ...
    while True:
        ...
        action = (
            input(f"{Fore.MAGENTA}Choose an action:{Style.RESET_ALL} ").strip().lower()
        )
        continue_game = True
        activity_log = ""
        if action == "1":
        ...
        elif action == "5" and isinstance(pet, Dog):
            continue_game, activity_log = pet.learn_trick()
        elif action == "5" and isinstance(pet, Cat):
        ...
        if not continue_game:
            ...
            activity_log = (
                f"{Fore.RED}🔄 Restarting... Choose better next time.{Style.RESET_ALL}"
            )
            display_menu(pet, activity_log)
            main()
            break
